...
Game.prototype.answer = function (playerToken, answer) {
  if (!this.currentState == this.WAITING_FOR_ANSWERS || !this.players[playerToken] || this.currentAnswers[playerToken]) {
    return;
  }
...
Statistics.prototype.updateDrinks = function (question, allPlayers, wrongPlayers) {
  ...
  for (var playerToken in allPlayers) {
    allPlayerIds.push(playerToken);
  }for (var index in wrongPlayers) {
    wrongPlayerIds.push(wrongPlayers[index].id);
